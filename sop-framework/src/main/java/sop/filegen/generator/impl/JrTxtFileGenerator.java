...
public class JrTxtFileGenerator extends BaseFileGenerator {
    ...
    protected GenResult generateFile(GenRequest request) throws FileGenerationException {
        ...
        try {
            ...
            outFile = JrReportEngine.getInstance().exportTxtWithJRDatasource(request.getReportId(), FileGeneratorUtils.getParamMap(request), jrds, outFile);
            result.setReturnCode(FileGeneratorConstant.SUCCESS);
            result.setReportFile(outFile);

        } catch (Exception e) {
            logger.error("TXT generation error:", e);
