...
public class JrXlsxFileGenerator extends BaseFileGenerator {
    ...
    protected GenResult generateFile(GenRequest request) throws FileGenerationException {
        ...
        try {
            ...
            outFile = JrReportEngine.getInstance().exportXlsxWithJRDatasource(request.getReportId(), FileGeneratorUtils.getParamMap(request), jrds, outFile);
            result.setReturnCode(FileGeneratorConstant.SUCCESS);
            result.setReportFile(outFile);

        } catch (Exception e) {
            logger.error("XLSX generation error:", e);
